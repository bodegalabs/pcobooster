...
4. Inspect the active `main` ruleset and open PRs before changing a required-check name. Change the ruleset only when the new check exists on an open PR and the migration will not strand another PR. During migration, keep the documented compatibility job. Completion: new PRs show `ci`, no older PR is stranded, and the remaining ruleset swap is explicit.
