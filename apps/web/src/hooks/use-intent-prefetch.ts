...
/**
 * Binds `createIntentPrefetcher` to a component. Pointer and keyboard focus behave the same:
 * each must rest on a target for the dwell time, and leaving cancels it. Touch pointers fire
 * the same events, so a tap or a scroll ends before the dwell and prefetches nothing; the
 * tap's navigation loads the target.
 */
export const useIntentPrefetch = <Target>(
  options: Omit<IntentPrefetcherOptions<Target>, "dwellMs">
) => {
