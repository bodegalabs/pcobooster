...
export interface PeopleDashboardPersonDependencies {
  ...
  readonly plansService: Pick<
    ...
    "getPlansInDateRange"
  >;
  readonly resolveTimeZone: (signal?: AbortSignal) => Promise<string>;
}
...
const getMonthRosterScheduleItems = async (
  ...
  const serviceTypes = await dependencies.catalogService.getServiceTypesCached(
    undefined,
    signal
  );
        ...
        plans.map(async (plan) => {
          const [members, planTimes] = await Promise.all([
            dependencies.peopleService
              .getPlanTeamMembers(serviceTypeId, plan.id, signal)
              .catch(() => {
                signal?.throwIfAborted();
                return { data: [], included: [] };
              }),
            dependencies.peopleService
              .getPlanPlanTimes(plan.id, signal)
              .catch(() => {
                signal?.throwIfAborted();
                return [];
              }),
...
const buildPeopleDashboardPerson = async ({
  ...
  const [personResource, schedulesResponse, monthRosterItems] =
    await Promise.all([
      peopleService.getPerson(personId, signal),
      getPersonSchedulesForDetail(peopleService, personId, signal),
...
export const getPeopleDashboardPerson = async (
  ...
  const orgTimeZone = await dependencies.resolveTimeZone(signal);
  const now = new Date();
