...
import type { SongOptionsReader } from "@pcobooster/api/modules/planning-center/get-song-options";
...
describe(getSongOptions, () => {
  it("prefers the last scheduled arrangement and key when available", async () => {
    ...
    getSongMock.mockReturnValue(
      Effect.succeed<SuccessOf<typeof getSongMock>>({
        id: "song-1",
        type: "Song",
        attributes: {
          title: "Build My Life",
    ...
    getSongArrangementsWithKeysMock.mockReturnValue(
      Effect.succeed<SuccessOf<typeof getSongArrangementsWithKeysMock>>({
        data: [
          {
            id: "arr-1",
            type: "Arrangement",
    ...
    getSongLastScheduledItemMock.mockReturnValue(
      Effect.succeed<SuccessOf<typeof getSongLastScheduledItemMock>>({
        data: {
          id: "item-1",
          type: "Item",
          attributes: {
    ...
    const options = await Effect.runPromise(
      getSongOptions("song-1", "service-1", songsReader)
    );
  ...
  it("falls back to the first non-archived arrangement and key", async () => {
    ...
    const options = await Effect.runPromise(
      getSongOptions("song-2", "service-1", songsReader)
    );
  ...
  it("builds key labels from starting and ending keys when the name is blank", async () => {
    ...
    const options = await Effect.runPromise(
      getSongOptions("song-3", "service-1", songsReader)
    );
