...
Schema migrations are generated by `Drizzle.Schema` on `bun run dev` (or explicitly with `bun run db:generate`); commit them. Alchemy applies them during local startup and deployments. No remote database credentials are needed to run tests.
