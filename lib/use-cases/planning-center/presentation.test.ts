...
vi.mock("@/lib/planning-center/services/people-service", () => ({
  planningCenterPeopleService: mocks,
}));
...
describe("presentation mode", () => {
  ...
  it("masks every person view consistently without changing source data or scheduling fields", async () => {
    ...
    const [candidates, roster, overview, personDetail] = await Promise.all([
      ...
      presentTeamPositions(groups),
      presentDashboard(dashboard),
      presentDashboardPerson(detail),
    ]);
    ...
    expect(personDetail.person.initials).toBe(
      candidates[0].firstName[0] + candidates[0].lastName[0]
    );
...
describe("people search", () => {
  ...
  it("keeps the existing upstream search and real avatar in normal mode", async () => {
    ...
    expect(await searchPeople("Private")).toEqual([
      {
        id: "person-1",
        firstName: "Private",
        lastName: "Name",
