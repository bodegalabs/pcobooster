...
Each stage's API declares a Cloudflare Flagship app and flags ([Feature flags](environment.md#feature-flags)), and planning reads them, so both deploy tokens also need the account-level **Flagship Write** permission group (it includes read). Alchemy 2.0.0-beta.79's typed permission catalog does not list Flagship yet, so `alchemy.ci.ts` must reference it by ID: add `{ id: "<Flagship Write permission group id>" }` to `deployPermissions`, with the ID from `GET /accounts/<account id>/tokens/permission_groups`. App-scoped Flagship tokens do not fit, because previews create their apps.
