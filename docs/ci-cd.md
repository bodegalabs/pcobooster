...
The production token has the same account-level deployment permissions, plus Zone Write, DNS Write, and Dynamic URL Redirects Write on every zone in the account. The `prod` stage owns two zones: `pcobooster.com` and the former `worshipadmin.com`, which it creates and answers with a redirect rule (see the [former domain cutover](cloudflare-cutover.md#former-domain-cutover)). Creating a zone needs Zone Write on all of the account's zones, so while the stack may create one, the grant cannot be narrowed to named zones. It also lets the token edit or delete any zone added to this account later; keep unrelated domains in another account. The token is stored only in the production Infisical project. `Cloudflare.state()` shares the bootstrapped Alchemy state Worker and Secrets Store across stages. Keep their credentials out of application bindings, artifacts, and logs.
