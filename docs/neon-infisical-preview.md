...
Infisical owns the seven required app keys in each environment: `DATABASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `PEOPLE_PAGE_ENABLED`, `PLANNING_CENTER_OAUTH_CLIENT_ID`, `PLANNING_CENTER_OAUTH_CLIENT_SECRET`, and `WORSHIP_ADMIN_ADMIN_EMAILS`. The Vercel records are separated by environment. Edit these keys in Infisical, not Vercel. All three syncs (`vercel-development`, `vercel-preview`, `vercel-production`) have automatic updates enabled. A changed synchronized value applies to new deployments; redeploy to update an existing deployment. [Vercel environment variables](https://vercel.com/docs/environment-variables/managing-environment-variables)
