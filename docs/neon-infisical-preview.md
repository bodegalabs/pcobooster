...
This is the shared-branch setup for `worshipadmin.com`. The three one-way syncs were activated and checked on September 16, 2026. See the [Infisical Vercel sync guide](https://infisical.com/docs/integrations/secret-syncs/vercel) for the connection controls.

| App environment | Infisical source | Vercel target | Neon project `worship-admin` branch |
| --- | --- | --- | --- |
...
Infisical owns the six app keys in each environment: `DATABASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `PLANNING_CENTER_OAUTH_CLIENT_ID`, `PLANNING_CENTER_OAUTH_CLIENT_SECRET`, and `WORSHIP_ADMIN_ADMIN_EMAILS`. The Vercel records are separated by environment; all 18 destination values were checked against Infisical after the initial sync. Edit these keys in Infisical, not Vercel. Vercel keeps its own `FLAGS` and `FLAGS_SECRET`; they are outside the Infisical sources and deletion protection is enabled. All three syncs (`vercel-development`, `vercel-preview`, `vercel-production`) have automatic updates enabled and show `Synced`. A changed Vercel variable applies to new deployments; redeploy to update an existing deployment. [Vercel environment variables](https://vercel.com/docs/environment-variables/managing-environment-variables)
...
The extra empty `worship-admin-preview` Neon project is unused and has not been deleted. Nothing is currently configured to provision branch-per-preview automatically; enabling that later would require a different ownership model for the dynamically assigned `DATABASE_URL`.
