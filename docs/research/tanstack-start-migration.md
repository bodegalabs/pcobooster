...
- **Presentation scope and `PEOPLE_PAGE_ENABLED` (sections 6 and 2).** Both are build-time `define`s in `vite.config.ts` instead of a root server function. They were build-time values under Next.js too: the build fingerprint and Alchemy stamp tracked `PEOPLE_PAGE_ENABLED`, and presentation mode exists only on the dev server. As constants, the scope is available synchronously when `getRouter` creates the query client, and no request pays a round trip. `<html data-presentation-scope>` is still rendered, and the browser caches read it as before.
