...
7. Allow agent-phase network access to `app.infisical.com`, `console.neon.tech`, `aws.neon.tech`, `googleapis.com`, and `gstatic.com`. Codex allowlist entries cover their subdomains, so the Neon entry includes region-qualified database hosts and the Google entries include the configured font endpoints. Keep all HTTP methods enabled because branch creation, SQL-over-HTTP migrations, and deletion use non-GET requests. Setup already has internet access for package installs.
